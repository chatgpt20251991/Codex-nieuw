...
export class ResolverController {constructor(private readonly prisma:PrismaService,private readonly resolver:ResolverService){}
 @Public() @Get(':publicId') async passport(@Param('publicId') publicId:string){const rows=await this.prisma.$queryRaw<any[]>`SELECT get_public_passport_snapshot(${publicId}) AS data`;const data=rows?.[0]?.data;if(!data)throw new NotFoundException({code:'PUBLIC_PASSPORT_NOT_FOUND'});return projectPassport(data);}
