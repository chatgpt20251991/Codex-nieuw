...
export class PassportDataService {
  ...
  async loadMergedTx(tx:Prisma.TransactionClient,organisationId:string,itemId:string){
    const item=await tx.batteryItem.findFirstOrThrow({where:{id:itemId,organisationId},include:{model:{include:{values:{where:{validUntil:null,validationStatus:{not:'superseded'}},include:{evidenceLinks:true}}}},values:{where:{validUntil:null,validationStatus:{not:'superseded'}},include:{evidenceLinks:true}}}});
  ...
  async validate(organisationId:string,itemId:string){
    ...
    const input=values.map(v=>({fieldId:v.fieldDefinitionId,value:v.valueJson,unit:v.unit||undefined,validated:v.validationStatus==='validated',evidenceIds:v.evidenceLinks.map((x:any)=>x.evidenceId)}));
